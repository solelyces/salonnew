...
const Profile = () => {
  ...
  fetch(`http://localhost:3000/api/transactions?user_id=${userId}`)
    ...
    .then(data => {
      ...
      // If data is an object with an array inside, adjust accordingly
      if (Array.isArray(data)) {
        setBookings(data);
      } else if (data && Array.isArray(data.data)) {
        setBookings(data.data);
      } else {
        setBookings([]); // fallback if data is not as expected
      }
    })
    .catch(error => {
            ...
            <nav className="menu-nav">
              <ul className="nav flex-column" role="tablist">
                ...
                <li className="nav-item" id='logout'>
                  <a className="nav-link" color='red' data-bs-toggle="tab" href="#logout" onClick={handleLogout}>
      ...
      <div className="all-bookings-container">
        {bookings.map((booking, index) => (
          <div key={index} className="receipt">
            {/* Render individual booking info here */}
              ...
              <div className="service-item">
                ...
                <span className="service-price">${Number(booking.total).toFixed(2)}</span>
                 <div className="service-actions">
                                  <button className="btn-edit" onClick={() => handleEditService(booking.id, idx)}>Edit</button>
                                  <button className="btn-delete" onClick={() => handleDeleteService(booking.id, idx)}>Delete</button>
                              </div>
              </div>
            </div>
            ...
            <div className="booking-item">
              <span className="booking-name">{booking.Date}</span>
               <div className="booking-actions">
                              <button className="btn-edit" onClick={() => handleEditBookingDate(booking.id, idx)}>Edit</button>
                              <button className="btn-delete" onClick={() => handleDeletebookingDate(booking.id, idx)}>Delete</button>
                          </div>
            </div>
            {/* Booking Time */}
            ...
            <div className="booking-item">
              ...
              <div className="booking-actions">
                              <button className="btn-edit" onClick={() => handleEditBookingTime(booking.id, idx)}>Edit</button>
                              <button className="btn-delete" onClick={() => handleDeletebookingTime(booking.id, idx)}>Delete</button>
                          </div>
            </div>
            {/* Payment Method */}
