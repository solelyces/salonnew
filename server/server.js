...
// Endpoint to fetch all users in admin dashboard
...
// Endpoint to update user information in admin dashboard
...
// Endpoint to delete a user in admin dashboard
...
// Endpoint to fetch all services into booking page
...
app.get('/api/client/transactions-pending', (req, res) => {
  ...
  const sql = `
      ...
      WHERE t.user_id = ? AND t.status = 'pending' order by recordID desc;
  `;
...
app.get('/api/client/transactions-paid', (req, res) => {
  ...
  const sql = `
      ...
      JOIN services_info s ON t.services_id = s.services_id
      JOIN payment_info p ON t.paymentinfo_id = p.paymentinfo_id
      WHERE t.user_id = ? AND t.status IN ('paid', 'declined')
...
app.get('/api/admin/transactions-pending', async (req, res) => {
  ...
  try {
    ...
    const [rows] = await connection.promise().query(`
      ...
      INNER JOIN services_info s ON t.services_id = s.services_id
      INNER JOIN payment_info p ON t.paymentinfo_id = p.paymentinfo_id
      WHERE t.status = 'pending' order by t.recordID DESC
    `);
    res.json(rows);
  } catch (err) {
...
app.get('/api/admin/transactions-paid', async (req, res) => {
  ...
  try {
    ...
    const [rows] = await connection.promise().query(`
      ...
      INNER JOIN services_info s ON t.services_id = s.services_id
      INNER JOIN payment_info p ON t.paymentinfo_id = p.paymentinfo_id
      WHERE t.status = 'paid' order by t.recordID DESC
    `);
    res.json(rows);
  } catch (err) {
    console.error('Error:', err);
...
app.get('/api/admin/transactions-declined', async (req, res) => {
  const connection = mysql2.createConnection({ host: 'localhost', user: 'root', password: '', database: 'system' });
  try {
    await new Promise((resolve, reject) => {
      connection.connect((err) => {
    ...
    const [rows] = await connection.promise().query(`
      ...
      INNER JOIN services_info s ON t.services_id = s.services_id
      INNER JOIN payment_info p ON t.paymentinfo_id = p.paymentinfo_id
      WHERE t.status = 'declined' order by t.recordID DESC
    `);
    res.json(rows);
  } catch (err) {
    console.error('Error:', err);
